...
class cmb_Meta_Box {
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				/*** Code for new custom types video_duration and video_url ***/
				case 'duration':
					$duration_val=explode (":", $meta);
					if (count($duration_val)>1){
						$hours_v=$duration_val[0];
						$minutes_v=$duration_val[1];
						$seconds_v=$duration_val[2];
					}

					echo '<input class="cmb_text_tiny" maxlength="2" name="', $field['id'], '[hours]" id="', $field['id'], '_hours" value="', '' !== $meta ? $hours_v : $field['std'], '" />:';
					echo '<input class="cmb_text_tiny" maxlength="2" name="', $field['id'], '[minutes]" id="', $field['id'], '_minutes" value="', '' !== $meta ? $minutes_v : $field['std'], '" />:';
					echo '<input class="cmb_text_tiny" maxlength="2" name="', $field['id'], '[seconds]" id="', $field['id'], '_seconds" value="', '' !== $meta ? $seconds_v : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					echo '<br><span id=result></span><br><span id=result_m></span><br><span id=result_s></span>';
					break;
				case 'video_url':
					echo '<input type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					echo '<span id=result_></span>';
					break;
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			/*** code for new type duration ***/
			if ($field['type']=='duration' && $type_comp==true)
				$new=$new['hours'].":". $new['minutes'] . ":". $new['seconds'];
...
function cmb_scripts( $hook ) {
	...
	if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
		...
		//Adding js files for video_duration and video_url
		//$template_dir =  get_bloginfo('template_directory');
		//wp_register_script( 'cmb-video', $template_dir . '/js/validate_yp7_tmpl_add_video_fields.js');
		//wp_enqueue_script( 'cmb-video' );
